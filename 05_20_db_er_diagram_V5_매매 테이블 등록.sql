...
CREATE TABLE IF NOT EXISTS `happyhouse`.`imgboard` (
  ...
  `save_path` VARCHAR(100) NOT NULL COMMENT '파일 저장 경로',
  `size` VARCHAR(45) NOT NULL COMMENT '파일 크기',
  `delete_yn` VARCHAR(1) NOT NULL COMMENT '삭제 여부',
  `created_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 시간',
  `delete_time` TIMESTAMP NULL COMMENT '삭제 시간',
  PRIMARY KEY (`no`),
...
alter table threadboard drop opt;
alter table threadboard drop address;